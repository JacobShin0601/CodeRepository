...
def compute_probabilities(X, theta, temp_parameter):
    ...
    """
    #YOUR CODE HERE
    vector_before_exp = (theta@X.T)/temp_parameter
    #print(vector_before_exp)
    # print(vector_before_exp.shape)
    c = np.max(vector_before_exp, axis=0)
    # print(c)
    # print(vector_before_exp)
    vector = np.exp(vector_before_exp - c)
    #print(vector)
    numerator = np.sum(vector, axis=0)
    #print(vector)
    
    
    H = vector / np.sum(vector, axis=0)
    #print(H)
    return H
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    """
    #YOUR CODE HERE
    # trans_X = np.merge(np.zeros_like(X[0]), X)
    # train_y = theta@X.T
    # print(train_y)
    
    pred_Y = get_classification(X, Y, temp_parameter)
    test_error = (1 - np.remainder(pred_Y, 3)==Y).mean()
    return test_error
