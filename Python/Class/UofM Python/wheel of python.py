...
class WOFPlayer:
    ...
    def __str__(self):
        prizeMoney = str(self.prizeMoney)
        sentence = self.name + " ($" + prizeMoney + ")"
        return sentence
...
""".format(self.name, self.prizeMoney, category, obscured_phrase, guessed))
        ...
        response = input("Guess a letter, phrase, or type 'exit' or 'pass':")
        if(response == 'exit'):
            return 0
        elif(response == 'pass'):
            return 'pass'
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        ...
        guessed_str = guessed_str.upper()

        for letter in LETTERS:
            if (letter not in guessed_str):
                filtered_LETTERS += letter

        if self.prizeMoney < VOWEL_COST:
            for vowel in VOWELS:
                filtered_LETTERS = filtered_LETTERS.replace(vowel, '')
            #filtered_LETTERS = filtered_LETTERS.replace(VOWELS, '')
    ...
    def getMove(self, category, obscured_Phrase, guessed):
        ...
        bool = self.smartCoinFlip()
        if bool == True:
            freq_str = self.SORTED_FREQUENCIES
            reversed_freq_str = reversed(freq_str)
            for char in reversed_freq_str:
                if char in possible_letters:
                    return char
        elif bool == False:
            selected_char = random.choice(possible_letters)
